...
def add_expense(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('consumptionANDincome:expense_statistic')  # 添加成功后重定向到支出统计页面
    else:
...
def delete_expense(request, expense_id):
    ...
    return redirect('consumptionANDincome:expense_statistic')  # 删除成功后重定向到支出统计页面
...
def update_expense(request, expense_id):
    try:
        ...
        return redirect('consumptionANDincome:expense_statistic')  # 如果没有找到对应的支出项，跳转到支出统计页面

    if request.method == 'POST':
        form = ExpenseEntryForm(request.POST, instance=expense)
        if form.is_valid():
            form.save()
            return redirect('consumptionANDincome:expense_statistic')  # 更新成功后重定向到支出统计页面
    else:
...
def add_income(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('consumptionANDincome:income_statistic')  # 添加成功后重定向到收入统计页面
    else:
...
def delete_income(request, income_id):
    ...
    return redirect('consumptionANDincome:income_statistic')  # 删除成功后重定向到收入统计页面
...
def update_income(request, income_id):
    try:
        ...
        return redirect('consumptionANDincome:income_statistic')  # 如果没有找到对应的收入项，跳转到收入统计页面

    if request.method == 'POST':
        form = IncomeEntryForm(request.POST, instance=income)
        if form.is_valid():
            form.save()
            return redirect('consumptionANDincome:income_statistic')  # 更新成功后重定向到收入统计页面
    else:
